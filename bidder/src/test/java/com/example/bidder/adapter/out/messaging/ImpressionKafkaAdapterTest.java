...
@Testcontainers
@SpringBootTest(classes = {ImpressionKafkaAdapter.class, KafkaProducerConfig.class})
class ImpressionKafkaAdapterTest {
  ...
  @Autowired
  private ImpressionKafkaAdapter kafkaAdapter;
  private KafkaConsumer<String, KafkaImpressionLog> testConsumer;
  ...
  static void setUp() {
    ...
    try (CachedSchemaRegistryClient schemaRegistryClient = new CachedSchemaRegistryClient(
      ...
      schemaRegistryClient.register("impression-log", avroSchema);
  ...
  void shouldProduceMessageWithSchema() {
    ...
    kafkaAdapter.sendImpression(impression); // (send 메서드가 Avro 객체를 받는다고 가정)

    // then: 테스트 컨슈머가 메시지를 수신했는지 검증
    ConsumerRecords<String, KafkaImpressionLog> records = testConsumer.poll(Duration.ofSeconds(10));
