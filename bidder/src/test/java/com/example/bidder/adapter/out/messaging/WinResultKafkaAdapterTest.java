...
@Testcontainers
@SpringBootTest(classes = {WinResultKafkaAdapter.class, KafkaProducerConfig.class})
class WinResultKafkaAdapterTest {
  ...
  @Autowired
  private WinResultKafkaAdapter kafkaAdapter;
  private KafkaConsumer<String, KafkaWinLog> testConsumer;
  ...
  static void setUp() {
    ...
    try (CachedSchemaRegistryClient schemaRegistryClient = new CachedSchemaRegistryClient(
      ...
      AvroSchema avroSchema = new AvroSchema(new Parser().parse(winLogScheme));
      schemaRegistryClient.register("win-log", avroSchema);
  ...
  void shouldProduceMessageWithSchema() {
    ...
    kafkaAdapter.sendWinResult(win); // (send 메서드가 Avro 객체를 받는다고 가정)

    // then: 테스트 컨슈머가 메시지를 수신했는지 검증
    ConsumerRecords<String, KafkaWinLog> records = testConsumer.poll(Duration.ofSeconds(10));

    assertThat(records.isEmpty()).isFalse();

    ConsumerRecord<String, KafkaWinLog> receivedRecord = records.iterator().next();
